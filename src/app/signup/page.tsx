...
export default function SignupPage() {
    ...
    const handleSignUp = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        // This is a mock implementation. In a real app, you would create a new user.
        // For this mock, we'll just log the user in as the normal user.
        try {
            const success = await login('user@example.com', 'userpassword');
            if(success) {
                toast({ title: "Erfolg", description: "Konto erfolgreich erstellt. Sie sind jetzt eingeloggt." });
                router.push('/');
            } else {
                 toast({ title: "Registrierung fehlgeschlagen", description: "Konto konnte nicht erstellt werden.", variant: "destructive" });
            }
    ...
    const handleGoogleSignIn = async () => {
        setIsGoogleLoading(true);
         try {
            await googleLogin();
            toast({ title: "Erfolg", description: "Erfolgreich mit Google registriert." });
            router.push('/');
        } catch (error: any) {
        ...
        <motion.div variants={itemVariants}>
            <Card className="w-full max-w-md shadow-2xl">
                <CardHeader className="text-center">
                    <CardTitle className="font-headline text-4xl md:text-5xl">Konto erstellen</CardTitle>
                    <CardDescription>Treten Sie der Olivare-Familie bei und entdecken Sie eine Welt des Geschmacks.</CardDescription>
                </CardHeader>
                <CardContent>
                    <form onSubmit={handleSignUp} className="space-y-6">
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                ...
                                <Input id="name" type="text" placeholder="John Doe" className="pl-10" value={name} onChange={(e) => setName(e.target.value)} />
                            </div>
                        </div>
                        <div className="space-y-2">
