...
export function ProductDialog({
  ...
  const onSubmit = async (data: ProductFormValues) => {
    setIsUploading(true);
    // Use placeholder image (Firebase upload removed)
    let imageUrl =
      product?.images[0]?.src || "https://placehold.co/600x800.png";
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Füllen Sie die folgenden Details aus, um{" "}
            {product ? "das" : "ein neues"} Produkt{" "}
            {product ? "zu aktualisieren" : "hinzuzufügen"}.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 pt-4">
          ...
          <div>
            ...
            <Input
              id="name"
              {...register("name")}
              placeholder="z.B. Olivare Signature Blend"
              disabled={isUploading}
            />
            {errors.name && (
